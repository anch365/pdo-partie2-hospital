...
if ($_SERVER['REQUEST_METHOD'] !== "POST") {
    header("Location: ../public/modifier-rendezvous.php?error=bad-method");
    exit();
}
...
if (!isset($_POST["id"]) || !isset($_POST["patient_id"]) || !isset($_POST["appointment_datetime"])) {
    header("Location: ../public/modifier-rendezvous.php?error=missing-value");
    exit();
}
...
if (empty($_POST["id"]) || empty($_POST["patient_id"]) || empty($_POST["appointment_datetime"])) {
    header("Location: ../public/modifier-rendezvous.php?error=value-empty");
    exit();
}
...
$patientId = htmlspecialchars(strip_tags(trim($_POST["patient_id"])));
...
$request = $db->prepare("UPDATE appointments SET appointments.patient_id = :patient_id, appointment_datetime = :appointment_datetime WHERE id = :id");
